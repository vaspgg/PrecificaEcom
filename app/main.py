...
def pricing_form(title='',ncm='',category_id='',source='manual',marketplace='ML',listing_type_id='gold_special',price_ref=0,uf_destino='',logistic_type='not_specified',shipping_mode='not_specified',item_id='',origem_mercadoria=0,consumidor_final=True,contribuinte_icms=False):
 ...
 fiscal=calcular_fiscal_hibrido(regime=regime,receita_12m=receita,uf_origem=uf_origem,uf_destino=uf_destino,ncm=ncm,origem_mercadoria=origem_mercadoria,consumidor_final=consumidor_final,contribuinte_icms=contribuinte_icms);imposto=fiscal['imposto_pct'];fiscal_calc_note=fiscal['nota'];fiscal_partes=fiscal['partes']
 comissao=0.0;tarifa=0.0;frete_auto=0.0;fee_note='';shipping_note=''
 if marketplace=='ML' and category_id and price_ref and float(price_ref)>0:
  try:
   cfg=ensure_ml_token();fee=listing_price(cfg['access_token'] if cfg else None,float(price_ref),category_id,listing_type_id,logistic_type,shipping_mode)
   comissao=float(fee.get('percentage_fee') or 0);tarifa=float(fee.get('fixed_fee') or 0)
   if item_id and cfg and cfg['access_token'] and cfg['user_id']:
    try:
     frete_auto,_=shipping_quote(cfg['access_token'],cfg['user_id'],item_id,float(price_ref),listing_type_id,logistic_type,shipping_mode,True);shipping_note=f"Frete automático Mercado Envios: R$ {frete_auto:.2f}."
    except Exception as sx: shipping_note="Frete automático não disponível para este anúncio/conta. Informe o frete manualmente."
   fee_note=f"Taxas consultadas no Mercado Livre para preço de referência R$ {float(price_ref):.2f}, logística {logistic_type} e envio {shipping_mode}."
 ...
 ml_sel='selected' if marketplace=='ML' else '';sh_sel='selected' if marketplace=='SHOPEE' else '';cl_sel='selected' if listing_type_id=='gold_special' else '';pr_sel='selected' if listing_type_id=='gold_pro' else '';log_opts=''.join(f"<option value='{v}' {'selected' if logistic_type==v else ''}>{n}</option>" for v,n in [('not_specified','Não especificada'),('self_service','Mercado Envios Flex'),('xd_drop_off','Agência / ponto de despacho'),('cross_docking','Coleta Mercado Livre'),('fulfillment','Mercado Envios Full')]);ship_opts=''.join(f"<option value='{v}' {'selected' if shipping_mode==v else ''}>{n}</option>" for v,n in [('not_specified','Não especificado'),('me2','Mercado Envios'),('custom','Envio personalizado')])
 rfb_note=fiscal.get('nota_rfb','')
 if not fiscal.get('rfb_local_disponivel'):
  rfb_note="Motor fiscal oficial RFB não instalado nesta versão; cálculo atual usa o motor fiscal interno."
 fiscal_note=f"Empresa: {html.escape(regime)} | origem {html.escape(uf_origem)} → destino {html.escape(uf_destino)}. {html.escape(fiscal_calc_note)} {html.escape(rfb_note)}"
 return page(f"""<div class='card'><h1>Precificação</h1><p class='muted'>Produto: <b>{html.escape(title or 'Não informado')}</b>{' | NCM: <b>'+html.escape(ncm)+'</b>' if ncm else ''}{' | Categoria ML: <b>'+html.escape(category_id)+'</b>' if category_id else ''}</p><form method='get' action='/tabela-precificacao'><input type='hidden' name='title' value='{html.escape(title)}'><input type='hidden' name='ncm' value='{html.escape(ncm)}'><input type='hidden' name='category_id' value='{html.escape(category_id)}'><input type='hidden' name='item_id' value='{html.escape(item_id)}'><div class='grid'><div><label>Plataforma de venda</label><select name='marketplace'><option value='ML' {ml_sel}>Mercado Livre</option><option value='SHOPEE' {sh_sel}>Shopee</option></select></div><div><label>Tipo de anúncio</label><select name='listing_type_id'><option value='gold_special' {cl_sel}>Clássico</option><option value='gold_pro' {pr_sel}>Premium</option></select></div><div><label>Preço de venda / referência (R$)</label><input type='number' step='.01' min='.01' name='price_ref' value='{float(price_ref or 0) if price_ref else ''}' required></div><div><label>UF destino</label><input name='uf_destino' maxlength='2' value='{html.escape(uf_destino)}' required></div><div><label>Tipo de logística</label><select name='logistic_type'>{log_opts}</select></div><div><label>Modo de envio</label><select name='shipping_mode'>{ship_opts}</select></div><div><label>Origem da mercadoria</label><select name='origem_mercadoria'><option value='0' {'selected' if str(origem_mercadoria)=='0' else ''}>0 - Nacional</option><option value='1' {'selected' if str(origem_mercadoria)=='1' else ''}>1 - Estrangeira/importação direta</option><option value='2' {'selected' if str(origem_mercadoria)=='2' else ''}>2 - Estrangeira/adquirida no mercado interno</option></select></div><div><label>Destinatário</label><select name='contribuinte_icms'><option value='false' {'selected' if not contribuinte_icms else ''}>Consumidor final não contribuinte ICMS</option><option value='true' {'selected' if contribuinte_icms else ''}>Contribuinte do ICMS</option></select></div></div><br><button>Consultar taxas e impostos</button></form><div class='info'>{html.escape(fee_note)}</div>{f"<div class='info'>{html.escape(shipping_note)}</div>" if shipping_note else ''}<div class='info'>{fiscal_note}</div></div><div class='card'><h2>Custos e margem</h2><form method='post' action='/calcular-preco'><input type='hidden' name='title' value='{html.escape(title)}'><input type='hidden' name='ncm' value='{html.escape(ncm)}'><input type='hidden' name='category_id' value='{html.escape(category_id)}'><div class='grid'><div><label>Custo do produto (R$)</label><input type='number' step='.01' min='0' name='custo_produto' required></div><div><label>Embalagem (R$)</label><input type='number' step='.01' min='0' name='embalagem' value='0'></div><div><label>Outros custos (R$)</label><input type='number' step='.01' min='0' name='outros_custos' value='0'></div><div><label>Frete pago pelo vendedor (R$)</label><input type='number' step='.01' min='0' name='frete_vendedor' value='{frete_auto:.2f}'></div><div><label>Tarifa fixa marketplace (R$)</label><input type='number' step='.01' min='0' name='tarifa_fixa' value='{tarifa:.2f}'></div><div><label>Impostos (%)</label><input type='number' step='.01' min='0' name='imposto_pct' value='{"" if imposto is None else f"{imposto:.4f}"}'></div><div><label>Comissão marketplace (%)</label><input type='number' step='.01' min='0' name='comissao_pct' value='{comissao:.4f}'></div><div><label>Ads (%)</label><input type='number' step='.01' min='0' name='ads_pct' value='0'></div><div><label>Margem desejada (%)</label><input type='number' step='.01' min='0' name='margem_pct' value='20' required></div></div><br><button>Calcular preço de venda</button></form></div>""")
@app.get('/precificar')
...
def tabela_precificacao(title:str='',ncm:str='',category_id:str='',marketplace:str='ML',listing_type_id:str='gold_special',price_ref:float=0,uf_destino:str='',logistic_type:str='not_specified',shipping_mode:str='not_specified',item_id:str='',origem_mercadoria:int=0,contribuinte_icms:bool=False): return pricing_form(title,ncm,category_id,'manual',marketplace,listing_type_id,price_ref,uf_destino,logistic_type,shipping_mode,item_id,origem_mercadoria,True,contribuinte_icms)
