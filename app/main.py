...
def pricing_form(title='',ncm='',category_id='',source='manual',marketplace='ML',listing_type_id='gold_special',price_ref=0,uf_destino='',logistic_type='not_specified',shipping_mode='not_specified'):
 ...
 if marketplace=='ML' and category_id and price_ref and float(price_ref)>0:
  try:
   ...
   comissao=float(fee.get('percentage_fee') or 0);tarifa=float(fee.get('fixed_fee') or 0);fee_note=f"Taxas consultadas no Mercado Livre para preço de referência R$ {float(price_ref):.2f}, logística {logistic_type} e envio {shipping_mode}."
...
def similar_post(url:str=Form(...)):
 ...
 ship=i.get('shipping') or {};return pricing_form(i['title'] or '','',i['category_id'],'link','ML',i.get('listing_type_id') or 'gold_special',float(i.get('price') or 0),'',ship.get('logistic_type') or 'not_specified',ship.get('mode') or 'not_specified')
